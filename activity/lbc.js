...
function cb(error, response, html) {
  if (error) {
    ...
    let allCommentsBox = document.querySelector(
      ".mb-5.comments-container-body"
    );
    let firstMatchCommentBox = allCommentsBox.querySelector(".match-comment");
    let lastBallCommentBox = firstMatchCommentBox.querySelector(
      ".d-flex.match-comment-padder.align-items-center"
    );
    let lastBallComment = lastBallCommentBox.querySelector(
      ".match-comment-long-text > p"
    ).textContent;
